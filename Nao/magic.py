...
class VideoImage(threading.Thread):
	...
	def getImageInfo(self):
		...
		self.saveImage(image[6], image[0], image[1], 'RGB', 'PNG', str(image[5])+'_camImage.png')
		return str(image[5])+'_camImage.png'
	def saveImage(self, image_data, image_width, image_height, 
		...
		image = Image.frombytes(image_colorspace, (image_width, image_height), image_data)
		# Save the image.
...
def main(robot_IP, robot_PORT=9559):
	try:
		...
		#tts = ALProxy("ALTextToSpeech", 'nao.local', 9559)  # ip from stephan's wifi
		#tts.say("Hallo! I am ready to play!")
